...
from utils import logger, clean_number, parse_date
...
def parse_auctions_table(html: str) -> pd.DataFrame:
    ...
    if not table:
        raise ValueError("Таблица с аукционами не найдена на странице")
    
    dfs = pd.read_html(str(table), header=0, decimal=',', thousands=' ')
    if not dfs:
        raise ValueError("Не удалось прочитать таблицу")
    ...
    if 'format' in result.columns:
        ...
        logger.warning("Колонка 'format' отсутствует, не фильтруем")
